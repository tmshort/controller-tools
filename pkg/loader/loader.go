...
func LoadRootsWithConfig(cfg *packages.Config, roots ...string) ([]*Package, error) {
	...
	ourFlags := []string{"-tags", "ignore_autogenerated"}
	// Look at GOFLAGS for additional flags
	if goFlagsEnv := os.Getenv("GOFLAGS"); goFlagsEnv != "" {
		goFlags, err := quoted.Split(goFlagsEnv)
		if err != nil {
			ourFlags = append(ourFlags, goFlags...)
		}
	}
	l.cfg.BuildFlags = append(ourFlags, l.cfg.BuildFlags...)
